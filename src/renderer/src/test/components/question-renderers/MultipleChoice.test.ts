import MultipleChoiceVue from '@renderer/components/question-renderers/MultipleChoice.vue';
...
describe('multiple choice component', () => {
    ...
    const correctResponse = '63551a06-08fd-4c08-a11e-337e728bb1cf';
    ...
    it('renders available options with both text and resource content', async () => {
        const { getByText } = render(MultipleChoiceVue, {
            props: {
                options,
                correctResponse,
                questionContent: 'Select the valid statement.',
            },
        });
    ...
    it('emits update event when selected value changes', async () => {
        ...
        const { getByTestId, emitted } = render(MultipleChoiceVue, {
            props: {
                options,
                correctResponse,
                questionRef: '',
    ...
    it('displays success feedback and styles responses appropriately when the user answers correctly and answers are revealed', async () => {
        const { getByTestId } = render(MultipleChoiceVue, {
            props: {
                options,
                correctResponse,
                questionRef: '',
                ...
                modelValue: '63551a06-08fd-4c08-a11e-337e728bb1cf',
            },
        });
        ...
        options
            .filter((o) => o.uuid !== '63551a06-08fd-4c08-a11e-337e728bb1cf')
            .map((o) => o.uuid)
            .forEach((r) => {
